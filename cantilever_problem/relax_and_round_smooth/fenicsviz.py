...
from dolfin import UnitSquareMesh, VectorFunctionSpace, FunctionSpace, Function, plot as fenics_plot
...
def _build_spaces(n: int):
    ...
    Vu = VectorFunctionSpace(mesh, "CG", 1)      # state
    return mesh, Vc, Vu
...
def _as_state_function(u: np.ndarray, Vu: FunctionSpace) -> Function:
    ...
    expected = f.vector().size()
    if u_flat.size != expected:
        raise ValueError(
            f"Displacement array has length {u_flat.size}, "
            f"but the vector space expects {expected}.\n"
            f"You are probably loading an old scalar solution "
            f"(length should be 2 × number of nodes)."
        )

    f.vector().set_local(u_flat)
    f.vector().apply("insert")
    return f
...
def fenicsModel(
    ...
    perimeter = float(objectives["perimeter"])
