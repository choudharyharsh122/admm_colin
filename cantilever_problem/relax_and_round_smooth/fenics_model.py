...
def build_graph(n_x: int, n_y: int):
    """Build the same edge graph used by the OC implementation for TV/perimeter."""
    N = n_x * n_y
    graph = nx.Graph()
...
def build_scale(graph: nx.Graph):
    """Assign edge weights matching the OC implementation."""
...
def compute_tv(control: np.ndarray, graph: nx.Graph, scale: np.ndarray) -> float:
    """Compute a graph-based TV/perimeter metric on a discrete control."""
    diffs = []
...
eps = Constant(1.0e-6)  # epsilon used in the solid isotropic material
...
class Left(SubDomain):
    """Left edge – fully clamped (Dirichlet)."""
    def inside(self, x, on_boundary):
        return near(x[0], 0.0) and on_boundary

class RightMid(SubDomain):
    """Small segment around the mid-point of the right edge
       (used for a distributed traction that approximates a point load)."""
    def inside(self, x, on_boundary):
        # tolerance ≈ one element height
        tol = 1.0 / n
...
def forward(a, f0, bc):
    """
    Solve the linear-elasticity forward problem for a given density a(x).
    
    a   : density Function (DG0)
    f0  : total downward force (float)
    bc  : list of DirichletBCs defined on the VECTOR space V
    """
    # ---------------------------------------------------------------
    # IMPORTANT: use the VECTOR space V, never the old scalar space P
    # ---------------------------------------------------------------
    u = Function(P, name="Displacement")
    v = TestFunction(P)

    # Material
    E0  = 1.0          # base Young modulus
    nu  = 0.3          # Poisson ratio
    p   = 3.0          # SIMP penalty
    eps = 1e-6         # void stiffness (prevents singularity)

    # SIMP interpolation of Young modulus
    E = eps + (1.0 - eps) * a**p
    ...
    # Stress (plane strain, written with E and ν only)
    def sigma(w):
        eps_w = epsilon(w)
        factor = E / (1.0 - nu**2)
        return factor * (
            (1.0 - nu) * eps_w
            + nu * tr(eps_w) * Identity(2)
            + nu * eps_w          # wait – better use the matrix form above to avoid mistakes
        )
    ...
    class RightMid(SubDomain):
        def inside(self, x, on_boundary):
            tol = 1.0 / n
            return near(x[0], 1.0) and near(x[1], 0.5, tol) and on_boundary

    boundaries = MeshFunction("size_t", mesh, mesh.topology().dim() - 1)
    boundaries.set_all(0)
    RightMid().mark(boundaries, 2)
    ds = Measure("ds", domain=mesh, subdomain_data=boundaries)

    load_length = 2.0 / n
    traction = Constant((0.0, -f0 / load_length))

    # Residual
    F = inner(sigma(u), epsilon(v)) * dx - inner(traction, v) * ds(2)
...
if __name__ == "__main__":
    ...
    for alpha_val in alpha_list:
        ...
        for n in mesh_list:
            ...
            P = VectorFunctionSpace(mesh, "CG", 1)  # function space for solution
            ...
            # ------------------------------------------------------------------
            # Dirichlet BC – classic left-edge clamp (both components)
            # ------------------------------------------------------------------
            bc = [DirichletBC(P, Constant((0.0, 0.0)), Left())]

            # ------------------------------------------------------------------
            # Load
            #   Option A (recommended for continuum TO): distributed traction
            #   on a short segment of the right edge.  Magnitude is scaled so that
            #   the total force equals -f0.
            # ------------------------------------------------------------------
            f = 1.0                                    # total downward force
            load_length = 2.0 / n                       # approximate length of marked segment
            traction = Constant((0.0, -f / load_length))
            ...
            lb = 0.0
            ub = 1.0
            # compliance (the quantity you usually minimise)
            compliance = inner(traction, u) * ds(2)

            # if you need a Form object
            F_form = compliance
            #F_form  = f*u*dx          
            ...
            # Compute a graph-based perimeter on the final discrete control.
            graph = build_graph(n, 2 * n)
            scale = build_scale(graph)
            perimeter = compute_tv(np.asarray(a_disc, dtype=float), graph, scale)
            
            # Compute objectives for continuous and discrete
            u_opt = forward(a_opt, f, bc)
            
            F_form_cont = inner(traction, u_opt) * ds(2)
            ...
            F_form_disc = inner(traction, u_disc) * ds(2)
            ...
            with h5py.File(h5_path, "a") as h5f:
                ...
                save("perimeter", np.float64(perimeter))
                save("tv_disc", np.float64(perimeter))
