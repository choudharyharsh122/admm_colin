...
def _transpose_control_triangles(a: np.ndarray, n_x: int, n_y: Optional[int] = None) -> np.ndarray:
    """
    ...
    if a.size != 2 * n_x * n_y:
        raise ValueError(f"Expected control length {2*n_x*n_y}, got {a.size}")
    if n_x != n_y:
        raise ValueError("Diagonal transpose is only defined for square meshes")

    out = np.empty_like(a)
    for I in range(n_x):
        for J in range(n_y):
            src = 2 * (J + I * n_x)      # swapped
            dst = 2 * (I + J * n_x)
            out[dst] = a[src]          # L
            out[dst + 1] = a[src + 1]  # U
    return out
...
def plot_control_field(
    ...
    dim: int,
    *,
    mesh_dim_x: Optional[int] = None,
    mesh_dim_y: Optional[int] = None,
    ax=None,
    title: str = "Control",
    figsize=(5, 4),
    show: bool = True,
):
    mesh_dim_x = 2 * int(dim) if mesh_dim_x is None else int(mesh_dim_x)
    mesh_dim_y = int(dim) if mesh_dim_y is None else int(mesh_dim_y)

    mesh, Vc, _ = _build_fenics_spaces(mesh_dim_x, mesh_dim_y)
    a = np.asarray(control_vec, dtype=float).ravel()

    expected = 2 * mesh_dim_x * mesh_dim_y
    if a.size != expected:
        raise ValueError(
            f"Control length mismatch: expected {expected} (=2*{mesh_dim_x}*{mesh_dim_y}), got {a.size}"
        )
...
def _build_fenics_spaces(mesh_dim_x: int, mesh_dim_y: int):
    """
    Same spaces as fenicsModel:
            DG0 for control (2*mesh_dim_x*mesh_dim_y dofs on UnitSquareMesh(mesh_dim_x,mesh_dim_y))
            CG1 for state   ((mesh_dim_x+1)*(mesh_dim_y+1) dofs)
    """
    mesh = UnitSquareMesh(mesh_dim_x, mesh_dim_y)
...
class ADMMColin:
    ...
    def __init__(
        ...
        *,
        ...
        self.dim = int(dim)
        self.mesh_dim_x = 2 * self.dim
        self.mesh_dim_y = self.dim
    ...
    def _load_summary(self) -> None:
        with h5py.File(self.h5_path, "r") as h5f:
            ...
            # Mesh dimensions may be stored per-seed in the run file metadata.
            # Fallback to the standard 2:1 convention if unavailable.
            try:
                grp_med = h5f[self.median_seed_name]
                self.mesh_dim_x = int(grp_med.attrs.get("mesh_dim_x", self.mesh_dim_x))
                self.mesh_dim_y = int(grp_med.attrs.get("mesh_dim_y", self.mesh_dim_y))
            except Exception:
                pass
    ...
    def plot_control(
        ...
        if fix_diagonal_reflection:
            a = _transpose_control_triangles(a, self.mesh_dim_x, self.mesh_dim_y)


        plot_control_field(
            a,
            dim=self.dim,
            mesh_dim_x=self.mesh_dim_x,
            mesh_dim_y=self.mesh_dim_y,
            title=title,
            figsize=figsize,
            ax=ax,
            show=show,
    ...
    show: bool = True,
        ...
        if transpose:
            u = u.reshape((self.mesh_dim_y + 1, self.mesh_dim_x + 1)).T.ravel()
