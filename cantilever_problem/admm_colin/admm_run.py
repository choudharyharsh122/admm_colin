...
def save_data(
    ...
    with h5py.File(h5_path, "a") as h5f:  # 'a' = append mode
        ...
        seed_group.attrs["mesh_dim_x"] = int(mesh_dim_x)
        seed_group.attrs["mesh_dim_y"] = int(mesh_dim_y)
...
def run_trial(dim: int, idx: int, params) -> None:
    ...
    # 2:1 cantilever-style rectangle.
    mesh_dim_x = int(dim)
    mesh_dim_y = int(dim)
    graph_n_x = mesh_dim_y
    graph_n_y = 2 * mesh_dim_x
    ...
    for (i, seed_i) in enumerate(seeds):
        ...
        # --- ADMM variables
        dv = DesignVariables(seed=42, n_x=graph_n_x, n_y=graph_n_y, Vmax=V_max)
        ...
        # --- Create mesh and BC ---
        mesh = generate_unit_square_mesh(mesh_dim_x, mesh_dim_y)
        ...
        # print("dirichlet_nodes (first 20):", bc.dirichlet_nodes[:20])
        # print("total fixed DOFs:", len(bc.dirichlet_nodes))
        # print("unique nodes that are fixed:", len(np.unique(bc.dirichlet_nodes // 2)))
        # print("dirichlet_values (unique):", np.unique(bc.dirichlet_values))
        # --- Create load vector ---
        # 2 DOFs (x, y) per node; forces are applied to the y-DOF (index 2*node + 1).
        source_type = str(getattr(params, "SOURCE_TYPE", "tip")).strip().lower()
        n_nodes = mesh.coords.shape[0]
        f_vector = np.zeros(2 * n_nodes, dtype=float)
        if source_type == "uniform":
            f_vector[1::2] = -float(params.SOURCE_STRENGTH)
        else:
            # Cantilever point load on right edge, mid-height (free node).
            nnx = mesh_dim_x + 1
            load_i = mesh_dim_x
            load_j = mesh_dim_y // 2
            load_node = int(load_i + load_j * nnx)
            f_vector[2 * load_node + 1] = -float(params.SOURCE_STRENGTH)

        # print("Force vector non-zeros:", np.where(np.abs(f_vector) > 1e-12)[0])
        # print("Force values:", f_vector[np.abs(f_vector) > 1e-12])
        ...
        sub1 = Subproblem1Solver(
            ...
            material=MaterialInterpolation(penal=3.0, eps=1e-6),
        )
        ...
        u_list.append(np.zeros(2 * (mesh_dim_x + 1) * (mesh_dim_y + 1)))
        ...
        k = 0
        min_admm_iters = int(getattr(params, "MIN_ADMM_ITERS", 5))

        while k < params.ITER_MAX and (k < min_admm_iters or infeas_list[-1] > params.INFEAS_TOL):
                # --- Subproblem 1 ---
                t0 = time.perf_counter()
                b_k1, u_k1, oc_track_sub1 = sub1.solve(
                    a_k,
                    b_k,
                    lam_k,
                    rho_k,
