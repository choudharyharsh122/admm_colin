...
class DesignVariables:
    ...
    """
    def __init__(self, seed, size=None, Vmax=0.4, n_x=None, n_y=None):
        np.random.seed(seed)
        if n_x is None or n_y is None:
            if size is None:
                raise ValueError("Provide either size or both n_x and n_y")
            n_x = int(size)
            n_y = 2 * int(size)
        ...
        #self.rho = 0.5
        self.a   = np.zeros(n)   # binary values {0,1}
        self.b   = 0.4*np.ones(n)
        self.lam = np.zeros(n)                              # real numbers
        #self.b = np.random.rand(2*size*size)    
        #self.lam = np.random.rand(2*size*size)                             # continuous [0,1]
