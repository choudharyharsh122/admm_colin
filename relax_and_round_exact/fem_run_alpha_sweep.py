"""Batch runner for exact relax-and-round alpha/mesh sweeps.

This script prepares alpha and mesh sweeps, then invokes fem_model.py.
It supports explicit alpha lists and generated alpha ranges.
"""
...
def _format_float_list(values: List[float]) -> str:
    # Keep values compact while preserving precision for CLI pass-through.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run alpha sweep for relax_and_round_exact/fem_model.py"
    )
    ...
    parser.add_argument("--source-strength", type=float, default=1.0)
    ...
    model_script = Path(__file__).resolve().parent / "fem_model.py"
    command = [
        sys.executable,
        str(model_script),
        "--alpha_list",
    ...
    print("=== Running exact relax-and-round sweep ===", flush=True)
