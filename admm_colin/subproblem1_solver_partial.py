...
class Subproblem1Solver:
    ...
    def compute_element_energy(
        ...
        element_indices : np.ndarray, optional
            If provided, compute energies only for these element indices.
        ...
        if element_indices is None:
            element_indices = np.arange(self.n_elems, dtype=int)
        else:
            element_indices = np.asarray(element_indices, dtype=int)

        ce = np.zeros(len(element_indices), dtype=float)

        for i, e in enumerate(element_indices):
            elem_nodes = self.elems[e]
            U_e = U[elem_nodes]
            
            # Energy: ce_e = U_e^T * KE_e * U_e
            ce[i] = U_e @ self.KE_all[e] @ U_e
    ...
    def update_design(
        ...
        active_indices : np.ndarray, optional
            Indices of the active elements being updated.
        volume_target : float, optional
            Total allowed volume for the active subset after subtracting frozen volume.
        ...
        if active_indices is None:
            active_indices = np.arange(len(b), dtype=int)
        else:
        ...
        if volume_target is None:
            volume_target = float(self.volfrac * np.sum(self.v))
        ...
        def trial_update(mu):
            c0 = -gprime * ce * b**2
            c1 = np.zeros_like(c0)
            c2 = (lam + mu * v_active - rho * a) / self.n_elems
        ...
        while float(np.dot(v_active, b_test)) > volume_target:
            mu_high *= 2.0
            b_test = trial_update(mu_high)
            if mu_high > 1e16:
                break
        ...
        while (mu_high - mu_low) > 1e-6:
            ...
            b_mid_volume = float(np.dot(v_active, b_mid))
            bisection_history.append((float(mu_mid), b_mid_volume))

            if b_mid_volume > volume_target:
                mu_low = mu_mid
            else:
                mu_high = mu_mid
    ...
    def solve(
        ...
        b_old2 = b.copy()
        b_old = b.copy()
        update_indices = np.arange(len(b), dtype=int)
        while change > self.tol and loop < self.maxiter:
            ...
            # Compute sensitivities only for active elements
            ce = self.compute_element_energy(U, b, element_indices=update_indices)
            
            # Update design
            fixed_indices = np.setdiff1d(np.arange(len(b), dtype=int), update_indices)
            fixed_volume = float(np.dot(self.v[fixed_indices], b[fixed_indices])) if fixed_indices.size else 0.0
            b_new, mu = self.update_design(
                a[update_indices],
                b[update_indices],
                lam[update_indices],
                rho,
                ce,
                active_indices=update_indices,
                volume_target=float(self.volfrac * np.sum(self.v) - fixed_volume),
            )
            b[update_indices] = b_new
            slow_velocity_indices = update_indices[np.abs(b_old[update_indices] - b_old2[update_indices]) < 1e-4]
            slow_ce_indices = update_indices[ce < 1e-6]

            # Start freezing only after the first two OC iterations and 5th ADMM iteration.
            if loop >= 2 and k > 5:
                # freeze design variables that have slow velocity or have very low sensitivity
                freeze_indices = np.union1d(slow_velocity_indices, slow_ce_indices)
                update_indices = np.setdiff1d(update_indices, freeze_indices)
                print(f"Iteration {loop}: number of frozen indices ={len(freeze_indices)}")
